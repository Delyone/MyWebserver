...
void WebServer::eventListen() {
    ...
    assert(ret != -1);
    //设置管道写端为非阻塞（缓冲区满时 部分定时器事件丢失，但定时器非必须，没关系）
...
void WebServer::dealwithread(int sockfd) {
    ...
    if(1 == m_actormodel) {
        ...
        //若监测到读事件，将该事件放入请求队列，（让工作线程读取和处理请求）
        ...
        if(users[sockfd].read_once()) {
            ...
            //若监测到读事件，（内核读完了）将该事件放入请求队列，（让工作线程处理请求）
...
void WebServer::eventLoop() {
    ...
    while(!stop_server) {
        ...
        //处理定时器为非必须事件（所以管道写端可以非阻塞），收到信号并不是马上处理
        //完成读写事件后，再进行处理
        if(timeout) {
            utils.timer_handler();
